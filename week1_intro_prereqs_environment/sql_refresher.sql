...
SELECT
	...
	taxi.total_amount,
	CONCAT(zones_pickup."Borough", ' / ', zones_pickup."Zone") AS pickup_location, -- capitalized fields must be in quotes in Postgres
	CONCAT(zones_dropoff."Borough", ' / ', zones_dropoff."Zone") AS dropoff_location -- capitalized fields must be in quotes in Postgres
...
-- Checking for LocationIDs *not* in the Taxi data that *are* in the Zones data
SELECT
	taxi.tpep_pickup_datetime,
	taxi.tpep_dropoff_datetime,
	taxi.total_amount,
...
FROM public.yellow_taxi_data AS taxi
WHERE
	taxi."DOLocationID" NOT IN (SELECT "LocationID" FROM public.zones) OR
	taxi."PULocationID" NOT IN (SELECT "LocationID" FROM public.zones)
LIMIT 100
;
...
WHERE
	...
	taxi."DOLocationID" NOT IN (SELECT "LocationID" FROM public.zones WHERE "LocationID" <> 142) OR
	taxi."PULocationID" NOT IN (SELECT "LocationID" FROM public.zones)
LIMIT 100
;
...
GROUP BY 
	dropoff_date,
	taxi."DOLocationID"
ORDER BY dropoff_date ASC, taxi."DOLocationID" ASC
